...
impl AddressingMode {
    /// 寻址成功返回 `Some((地址, 是否跨页))`
    pub fn addressing(&self, bus: &mut Bus) -> Result<(u16, bool)> {
        match self {
    ...
    pub fn read(&self, bus: &Bus, address: u16) -> Result<u8> {
    ...
    pub fn write(&self, bus: &mut Bus, address: u16, data: u8) -> Result<()> {
    ...
    fn implicit_addressing(_bus: &mut Bus) -> Result<(u16, bool)> {
        Ok((0, false))
    }
    #[allow(clippy::unnecessary_wraps)]
    fn accumulator_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        Ok((bus.registers().a as u16, false))
    }
    fn immediate_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
        Ok((bus.cpu_read(pc)? as u16, false))
    }
    fn absolute_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 2;
        let address = bus.cpu_read_word(pc)?;
        Ok((address, false))
    }
    fn absolute_x_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 2;
        let address = bus.cpu_read_word(pc)?;
    ...
    fn absolute_y_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 2;
        let address = bus.cpu_read_word(pc)?;
    ...
    fn zero_page_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
        Ok((bus.cpu_read(pc)? as u16, false))
    }
    fn zero_page_x_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
        Ok((
    ...
    fn zero_page_y_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
        Ok((
    ...
    fn relative_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
    ...
    fn indirect_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 2;
    ...
    fn indirect_x_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
    ...
    fn indirect_y_addressing(bus: &mut Bus) -> Result<(u16, bool)> {
        let pc = bus.registers().pc;
        bus.registers_mut().pc += 1;
